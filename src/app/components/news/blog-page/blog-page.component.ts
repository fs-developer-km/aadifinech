...
export class BlogPageComponent {
  ...
  showSuccess = false;
  successMode: 'admin' | 'whatsapp' = 'admin';
  ...
  // ⚠️ Confirm this route with backend — must save into the same
  // Partner collection that /api/partner/partners reads from.
  // private registerUrl = 'https://api.aadifintech.com/api/partner/register';
  private registerUrl = 'https://api.aadifintech.com/api/partner/register';
  ...
  resetForm() {
    this.formData = {
      name: '', mobile: '', email: '', profession: '',
      city: '', state: '', experienceYears: '', companyName: ''
    };
  }
  ...
  // ✅ Button 1: Save directly to Admin Panel (Partner table)
  sendToAdmin() {
    ...
    this.http.post(this.registerUrl, payload).subscribe({
      next: () => {
        ...
        this.successMode = 'admin';
        this.showSuccess = true;
        setTimeout(() => this.closeForm(), 4000);
      },
      error: (err) => {
        this.isSubmittingAdmin = false;
        console.error('❌ Partner save failed:', err);
        alert(err.error?.message || 'Failed to submit. Please try again or use WhatsApp instead.');
      }
    });
  }

  // ✅ Button 2: Open WhatsApp only, no backend call
  continueOnWhatsApp() {
    if (!this.validateForm()) return;

    let message = `Hello Aadi Fintech,
...
Mobile: ${this.formData.mobile.trim()}`;
    ...
    if (this.formData.profession) message += `\nProfession: ${this.formData.profession.trim()}`;
    if (this.formData.city) message += `\nCity: ${this.formData.city.trim()}`;
    if (this.formData.state) message += `\nState: ${this.formData.state.trim()}`;
    if (this.formData.experienceYears) message += `\nExperience: ${this.formData.experienceYears} years`;
    if (this.formData.companyName) message += `\nCompany: ${this.formData.companyName.trim()}`;
